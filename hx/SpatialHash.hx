...
class SpatialHash {
	...
	var gridWidth : Int;
	var gridHeight : Int;
	function new(list : EntitySet) {
		widthInTiles = 25;
		heightInTiles = 25;
		gridWidth = 25;
		gridHeight = 25;
		grid = Util.make2DArrayFn(widthInTiles, heightInTiles, function() : Array<Dynamic> {
			return [];
		}
);
...
;
	...
	/* Return the coordinates that the provided entity hashes to. Could be more than
	   ...
	   on a grid, will only exist in one coordinate. */	function getCoords(e : Entity) : Array<Dynamic> {
		...
		if((e.x + e.width) % gridWidth == 0) 
			endSlotX--;
		var endSlotY : Int = (e.y + e.height) / gridHeight;
		if((e.y + e.height) % gridHeight == 0) 
			endSlotY--;
		var slotX : Int = (e.x / gridWidth);
		while(slotX <= endSlotX) {
			var slotY : Int = (e.y / gridHeight);
